...
def _silent_music_launch(monkeypatch):
    ...
    monkeypatch.setattr("music_manager.cli.play_playlist._music_was_running", lambda: True)
    monkeypatch.setattr(
        "music_manager.cli.play_playlist.subprocess.run",
        lambda *a, **k: type("R", (), {"returncode": 0})(),
    )
    monkeypatch.setattr("music_manager.cli.play_playlist.time.sleep", lambda _s: None)
...
def test_play_playlist_runs_applescript(capsys: pytest.CaptureFixture) -> None:
    ...
    with patch("music_manager.cli.play_playlist.run_applescript", return_value="") as mock_script:
...
def test_dispatcher_routes_to_play_playlist() -> None:
    with patch("music_manager.cli.play_playlist.main", return_value=0) as mock_main:
