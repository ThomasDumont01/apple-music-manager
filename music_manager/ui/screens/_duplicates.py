"""Duplicates mixin — find and resolve duplicate tracks (library + playlists)."""
...
class DuplicatesMixin(_MixinBase):
    ...
    def _start_duplicates(self) -> None:
        """Launch find-duplicates: scan library + playlists, per-group review."""
        ...
        groups = find_duplicates(self._tracks_store) if has_identified else []
        prefs_path = self._paths.preferences_path if self._paths else ""
        if prefs_path and groups:
            ignored = load_ignored(prefs_path)
            groups = [group for group in groups if group_key(group) not in ignored]

        pl_groups = find_playlist_internal_duplicates(self._tracks_store)

        if not groups and not pl_groups:
            self._view = "summary"
            ...
            body = RichText()
            body.append(f"\n  {CHECK}  ", style="green")
            body.append(f"{DUP_NONE_FOUND if has_identified else DUP_NO_IDENTIFIED}\n")
            self._set_body(body)
        ...
        self._dup_idx = 0
        self._dup_cursor = 0
        self._dup_pl_groups = pl_groups
        self._dup_pl_idx = 0
        self._dup_pl_cursor = 0
        self._dup_phase = "library" if groups else "playlist"
        self._dup_result = {
            "removed": 0,
            "skipped": 0,
            "ignored": 0,
        ...
        if self._dup_phase == "library":
            self._dup_render_group()
        else:
            self._dup_render_pl_group()

    # ── Library phase ───────────────────────────────────────────────────────

    def _dup_render_group(self) -> None:
        """Show current library duplicate group for review."""
        ...
        if self._dup_idx >= len(self._dup_groups):
            if self._dup_pl_groups:
                self._dup_phase = "playlist"
                self._dup_pl_idx = 0
                self._dup_pl_cursor = 0
                self._dup_render_pl_group()
            else:
    ...
    def _dup_skip(self) -> None:
        """S key shortcut — skip current group (library or playlist phase)."""
        from music_manager.core.logger import log_event  # noqa: PLC0415

        if self._dup_phase == "playlist":
            group = self._dup_pl_groups[self._dup_pl_idx]
            log_event("dup_pl_skip", playlist=group.get("playlist_name", ""))
            self._dup_result["skipped"] += 1
            self._dup_pl_idx += 1
            self._dup_render_pl_group()
            return
    ...
    def _dup_preview(self) -> None:
        """Play preview of currently selected duplicate entry (library only)."""
        if self._dup_phase != "library":
            return
        group = self._dup_groups[self._dup_idx]
    ...
    def _dup_render_pl_group(self) -> None:
        """Show current playlist with internal duplicates for review."""
        from music_manager.ui.render import render_playlist_duplicate_group  # noqa: PLC0415
        from music_manager.ui.text import (  # noqa: PLC0415
            DUP_PL_DEDUPE,
            DUP_SKIP,
            DUP_TITLE,
        ...
        if self._dup_pl_idx >= len(self._dup_pl_groups):
            self._dup_finish()
            return

        self._view = "duplicates_playlist"
        group = self._dup_pl_groups[self._dup_pl_idx]
        actions = [DUP_PL_DEDUPE, DUP_SKIP]
        self._dup_pl_actions = actions
        self._set_header(render_sub_header(DUP_TITLE))
        self._set_body(
            render_playlist_duplicate_group(
                group,
                self._dup_pl_cursor,
                self._dup_pl_idx + 1,
                len(self._dup_pl_groups),
                actions,
            )
        )
        self._set_help(HELP_DUP_PL)

    def _dup_pl_move(self, direction: int) -> None:
        """Navigate within current playlist group (actions only)."""
        total = len(self._dup_pl_actions)
        self._dup_pl_cursor = (self._dup_pl_cursor + direction) % total
        self._dup_pl_refresh()

    def _dup_pl_refresh(self) -> None:
        """Re-render current playlist group."""
        from music_manager.ui.render import render_playlist_duplicate_group  # noqa: PLC0415
        ...
        self._set_body(
            render_playlist_duplicate_group(
                ...
                self._dup_pl_idx + 1,
                len(self._dup_pl_groups),
                self._dup_pl_actions,
            )
        )

    def _dup_pl_select(self) -> None:
        """Enter on playlist phase: dedupe or skip."""
        if self._dup_pl_cursor == 0:
            self._dup_pl_dedupe()
        else:
            self._dup_skip()

    @work(thread=True)
    def _dup_pl_dedupe(self) -> None:
        """Rebuild current playlist with each track kept only once."""
        from music_manager.core.logger import log_event  # noqa: PLC0415
        from music_manager.options.find_duplicates import dedup_playlist  # noqa: PLC0415
        from music_manager.ui.text import DUP_PL_REMOVING  # noqa: PLC0415

        self.app.call_from_thread(self._dup_pl_set_working, DUP_PL_REMOVING)

        group = self._dup_pl_groups[self._dup_pl_idx]
        playlist_name = group.get("playlist_name", "")
        ordered_ids = group.get("ordered_ids", [])

        removed = dedup_playlist(playlist_name, ordered_ids)
        ...
        self._dup_result["playlist_removed"] += removed
        self.app.call_from_thread(self._dup_pl_advance)

    def _dup_pl_set_working(self, message: str) -> None:
        """Show working status during playlist dedupe."""
        from music_manager.ui.render import render_modify_status  # noqa: PLC0415

        self._view = "dup_pl_removing"
        self._set_body(render_modify_status(message))
        self._set_help("")

    def _dup_pl_advance(self) -> None:
        """Move to next playlist group after dedupe."""
        self._dup_pl_idx += 1
        self._dup_pl_cursor = 0
        self._dup_render_pl_group()
    ...
    def _dup_finish(self) -> None:
        ...
        self._set_body(
            render_duplicates_summary(
                ...
                self._dup_result["playlist_removed"],
            )
        )
        self._set_help(HELP_BACK, with_newline=False)
        self._save_all()
